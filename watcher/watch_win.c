...
void WatchDirectory(char *dir) {
	...
	ovlEventHandle.hEvent = CreateEvent(
	...
	while (TRUE) {
		...
		case WAIT_OBJECT_0 + 1:
			...
			DWORD numRead;
			char XXX[BUFFER_SIZE] = "";
			if (!ReadFile(stopWatchHandle, XXX, BUFFER_SIZE*sizeof(TCHAR), &numRead, NULL)) {
				printf("[CGO] [ERROR] read from pipe: ReadFile failed (%d)\n", GetLastError());
			}

			// check if dir == buffer
			if (numRead > 0) {
				int i = strcmp(XXX, dir);
				if (i == 0) {
					...
					printf("XXX [CGO] [INFO] WatchDirectory(): [me:%s vs buffer:%s] ignore\n", dir, XXX);
				}
			}
